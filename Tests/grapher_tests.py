...
class ComplexGrapherTests(unittest.TestCase):
    ...
    def test_figure_6(self):
        ...
        png = cg.save_as_png("figure_6_multiplication_by_pure_imaginary_illustrated")
        verify(str(hash(png)), self.reporter)

    def test_figure_7(self):
        cg = grapher.ComplexGrapher()
        cg.illustrate_multiplication(complex(0, -2), complex(-3, 2))
        png = cg.save_as_png("figure_7_multiplication_by_pure_negative_imaginary_illustrated")
        verify(str(hash(png)), self.reporter)
